...
//            return documentRepository.findDocumentsAccessibleByUser(currentUser.getId());
        ...
        List<Integer> accessibleDocIds = accessControlLogicRepository
...
//            return documentRepository.findDocumentsAccessibleByUser(userId);
     ...
     */
    @Transactional(readOnly = true)
    public boolean hasAccessToDocument(int documentId) {
        User currentUser = getCurrentUser();
     ...
     */
    @Transactional(readOnly = true)
    public boolean hasAccessToDocument(Long userId, int documentId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new RuntimeException("User not found"));
     ...
     */
    @Transactional(readOnly = true)
    public List<Integer> getAccessibleDocumentIds() {
        User currentUser = getCurrentUser();
